    ...
    <div className="min-h-screen py-20">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {poems.map((poem, index) => (
            ...
            >
              <poem.icon size={32} className="text-teal-500 mb-4" />
              <h3 className="text-xl font-bold mb-2">{poem.title}</h3>
              <p className="text-teal-500 text-sm mb-3">{poem.theme}</p>
      ...
      {selectedPoem && (
        ...
        >
          ...
          >
            <selectedPoem.icon size={40} className="text-teal-500 mb-4" />
            <h2 className="text-2xl font-bold mb-2">{selectedPoem.title}</h2>
            <p className="text-teal-500 mb-6">{selectedPoem.theme}</p>
            ...
            <button
              onClick={() => setSelectedPoem(null)}
              className="mt-6 w-full py-2 rounded-lg bg-teal-500 text-white hover:bg-teal-600 transition-colors"
            >
              Close
            </button>
          </div>
